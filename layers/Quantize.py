...
class Quantize(nn.Module):
    def __init__(self, input_dim: int, vq_dim: int, num_embed: int, split_num:int, codebook_num: int=1, **kwargs):
        super(Quantize, self).__init__(**kwargs)
        # hyper-parameters
        self.split_num = split_num
        assert split_num == 8
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            phase = torch.angle(x_fft)
            phase -= phase[:, :, 1:2] * self.linear
            x_fft = magnitude * torch.exp(1j * phase)
            # x_recon = torch.fft.irfft(x_recon, dim=-1)
